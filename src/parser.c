...
// 栈深度分析结果：max 表示相对起始深度的最大深度；effect 表示净栈深度变化
typedef struct {
    size_t max;      // 相对起始深度达到的最大深度
    int32_t effect;  // 表达式执行完毕后的净栈深度变化
} opstack_depth_result_t;


// 前向声明
static int32_t analyze_value(am_ast_t *ast, am_value_t v, opstack_depth_result_t *out);
...
static int32_t is_builtin_varid(am_ast_t *ast, am_varid_t varid) {
    ...
    for (size_t i = 0; i < AM_GLOBAL_BUILTIN_VAR_NUM; i++) {
        if (wcscmp(name, AM_GLOBAL_BUILTIN_VAR[i]) == 0) {
            return (AM_BUILTIN_OPCODE_MAP[i] >= 0) ? 0 : -1;
        }
    }
    return -1;
}
...
static int32_t analyze_args_call(
    ...
    opstack_depth_result_t *out
) {
    if (!ast || !lst || !out) return -1;
    if (n_args < 0) return -1;
    if (start_index + (size_t)n_args > lst->length) return -1;

    size_t current = 0;
    size_t max = 0;

    for (size_t i = 0; i < (size_t)n_args; i++) {
        size_t idx = start_index + i;
        opstack_depth_result_t arg;
        if (analyze_value(ast, am_list_get(ast->alloc, lst, idx), &arg) != 0) return -1;

        size_t arg_max = current + arg.max;
        if (arg_max > max) max = arg_max;
        current += arg.effect;
    }
...
static int32_t analyze_begin(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    ...
    for (size_t i = 1; i < lst->length; i++) {
        opstack_depth_result_t expr;
        if (analyze_value(ast, am_list_get(ast->alloc, lst, i), &expr) != 0) return -1;

        size_t expr_max = current + expr.max;
...
// 分析 define 形式：(define var value)
static int32_t analyze_define(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;
    if (lst->length < 3) {
        out->max = 0;
        out->effect = 0;
    ...
    opstack_depth_result_t val;
    if (analyze_value(ast, am_list_get(ast->alloc, lst, 2), &val) != 0) return -1;
...
// 分析 set! 形式：(set! var value)
static int32_t analyze_set(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;
    if (lst->length < 3) {
        out->max = 0;
        out->effect = 0;
...
// 分析 if 形式：(if pred then [else])
static int32_t analyze_if(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;

    size_t max = 0;

    if (lst->length > 1) {
        opstack_depth_result_t pred;
        if (analyze_value(ast, am_list_get(ast->alloc, lst, 1), &pred) != 0) return -1;
        if (pred.max > max) max = pred.max;
    }

    if (lst->length > 2) {
        opstack_depth_result_t true_branch;
        if (analyze_value(ast, am_list_get(ast->alloc, lst, 2), &true_branch) != 0) return -1;
        if (true_branch.max > max) max = true_branch.max;
    }

    if (lst->length > 3) {
        opstack_depth_result_t false_branch;
        if (analyze_value(ast, am_list_get(ast->alloc, lst, 3), &false_branch) != 0) return -1;
        if (false_branch.max > max) max = false_branch.max;
    }

    out->max = max;
    out->effect = 1; // if 表达式留下一个结果在栈上
    return 0;
}


// 分析 cond 形式：(cond (c1 e1) (c2 e2) ... [(else en)])
static int32_t analyze_cond(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;

    size_t max = 0;

    for (size_t i = 1; i < lst->length; i++) {
        am_value_t clause_handle = am_list_get(ast->alloc, lst, i);
        if (!am_value_is_handle(clause_handle)) continue;

        am_value_t clause_val = am_ast_get_node(ast, am_value_to_handle(clause_handle));
        if (!am_value_is_ptr(clause_val)) continue;
        ...
        if (clause_lst->length > 0) {
            opstack_depth_result_t pred;
            if (analyze_value(ast, am_list_get(ast->alloc, clause_lst, 0), &pred) != 0) return -1;
            if (pred.max > max) max = pred.max;
        }
        if (clause_lst->length > 1) {
            opstack_depth_result_t branch;
            if (analyze_value(ast, am_list_get(ast->alloc, clause_lst, 1), &branch) != 0) return -1;
            if (branch.max > max) max = branch.max;
        }
    }

    out->max = max;
    out->effect = 1; // cond 表达式留下一个结果在栈上
    return 0;
}


// 分析 while 形式：(while pred body...)
static int32_t analyze_while(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;
    ...
    // while 体中的表达式当前编译器不插入 pop，效果与 lambda 体一致
    size_t current = 0;
    ...
    out->max = max;
    out->effect = 0; // while 不留下显式结果
    return 0;
}


// 分析 and / or 形式
static int32_t analyze_and_or(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;
...
// 分析 quote 形式：(quote expr)
static int32_t analyze_quote_form(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    (void)ast;
    (void)lst;
    out->max = 1;
    out->effect = 1;
...
// 分析 quasiquote 列表内部（包括 (quasiquote ...) 形式以及 QUASIQUOTE 类型 list）
static int32_t analyze_quasiquote_list(am_ast_t *ast, am_list_t *lst, size_t start_index, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;

    size_t current = 0;
    size_t max = 0;

    for (size_t i = start_index; i < lst->length; i++) {
        opstack_depth_result_t child;
        if (analyze_value(ast, am_list_get(ast->alloc, lst, i), &child) != 0) return -1;

        size_t child_max = current + child.max;
        if (child_max > max) max = child_max;
        current += child.effect;
    }

    // push count
    size_t n = lst->length > start_index ? lst->length - start_index : 0;
    current += 1;
    if (current > max) max = current;

    // concat 消耗 n+1 个值，压入 1 个结果列表
    if (current >= n + 1) {
...
// 分析 quasiquote 形式：`(quasiquote item ...)
static int32_t analyze_quasiquote_form(am_ast_t *ast, am_list_t *lst, opstack_depth_result_t *out) {
    if (!ast || !lst || !out) return -1;
    // 跳过 children[0] 的 quasiquote 符号
    return analyze_quasiquote_list(ast, lst, 1, out);
}
...
static int32_t analyze_application(am_ast_t *ast, am_handle_t handle, opstack_depth_result_t *out) {
    ...
    if (am_value_is_symbol(first)) {
        am_symbol_t sym = am_value_to_symbol(first);
        if (sym == am_value_to_symbol(AM_VALUE_KW_begin))        return analyze_begin(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_define))       return analyze_define(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_set))          return analyze_set(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_cond))         return analyze_cond(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_if))           return analyze_if(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_while))        return analyze_while(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_and))          return analyze_and_or(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_or))           return analyze_and_or(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_quote))        return analyze_quote_form(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_quasiquote))   return analyze_quasiquote_form(ast, lst, out);
        if (sym == am_value_to_symbol(AM_VALUE_KW_unquote)) {
            // unquote 仅在 quasiquote 上下文中有意义，保守处理
            out->max = 1;
            out->effect = 1;
            return 0;
        }
        if (sym == am_value_to_symbol(AM_VALUE_KW_import) ||
            ...
            // import / native 不产生运行时栈操作
            out->max = 0;
            out->effect = 0;
            return 0;
        }
        // 其它 symbol 作为可调用项（编译器会报错），保守 +1
        out->max = 1;
        out->effect = 1;
        return 0;
    }

    // 首项是待求值的 Application，需要进行 η 变换
    if (am_value_is_handle(first)) {
        am_value_t first_node_val = am_ast_get_node(ast, am_value_to_handle(first));
        if (am_value_is_ptr(first_node_val)) {
            am_object_t *first_obj = am_value_to_ptr(first_node_val);
            if (first_obj->type == AM_OBJECT_TYPE_LIST) {
                am_list_t *first_lst = (am_list_t *)first_obj;
                if (first_lst->type == AM_LIST_TYPE_APPLICATION) {
                    // η 变换：所有子表达式（包括首项）作为参数压栈，再调用临时 lambda
                    return analyze_args_call(ast, lst, 0, (int32_t)lst->length, 1, out);
                }
            }
        }
    }
    ...
    // 普通函数调用：压参数，call 消耗 n_args 并压入结果
    return analyze_args_call(ast, lst, 1, n_args, 1, out);
}
...
static int32_t analyze_value(am_ast_t *ast, am_value_t v, opstack_depth_result_t *out) {
    ...
    // 立即数：number/symbol/boolean/null/undefined/wchar/varid/iaddr/label 等
    ...
    am_list_t *lst = (am_list_t *)obj;
    switch (lst->type) {
        case AM_LIST_TYPE_LAMBDA:
        case AM_LIST_TYPE_QUOTE:
            // 在值位置：loadclosure 或 push，均 +1
            out->max = 1;
            out->effect = 1;
            return 0;

        case AM_LIST_TYPE_APPLICATION:
        case AM_LIST_TYPE_UNQUOTE:
            return analyze_application(ast, h, out);

        case AM_LIST_TYPE_QUASIQUOTE:
            return analyze_quasiquote_list(ast, lst, 0, out);
...
// 分析 lambda 函数体，entry_depth 为形参数量（进入体时参数已在栈上）
static size_t analyze_lambda_body(am_ast_t *ast, am_handle_t lambda_handle) {
    if (!ast) return SIZE_MAX;

    am_value_t node_val = am_ast_get_node(ast, lambda_handle);
    if (!am_value_is_ptr(node_val)) return SIZE_MAX;

    am_object_t *obj = am_value_to_ptr(node_val);
    if (obj->type != AM_OBJECT_TYPE_LIST) return SIZE_MAX;

    am_list_t *lst = (am_list_t *)obj;
    if (lst->type != AM_LIST_TYPE_LAMBDA) return SIZE_MAX;

    am_uint_t n_param = 0;
    if (lst->length > 1) {
        am_value_t n = lst->children[1];
        if (am_value_is_uint(n)) n_param = am_value_to_uint(n);
    }

    size_t n_body = 0;
    am_value_t *bodies = am_list_lambda_get_bodies(ast->alloc, lst, &n_body);
    if (n_body > 0 && !bodies) return SIZE_MAX;

    // lambda 体入口处，编译器会按逆序插入 n_param 条 store 指令将参数弹出。
    // 因此参数本身达到的深度为 n_param，之后体从深度 0 开始执行。
    size_t max = (size_t)n_param;
    size_t current = 0;

    // 当前编译器在 lambda 体相邻表达式之间不插入 pop（TODO），因此直接累加 effect
    for (size_t i = 0; i < n_body; i++) {
        opstack_depth_result_t expr;
        if (analyze_value(ast, bodies[i], &expr) != 0) {
            free(bodies);
            return SIZE_MAX;
        }
...
size_t am_parser_opstack_depth_analysis(am_ast_t *ast) {
    ...
    size_t max_depth = 1; // 至少为 1，简单字面量也会压栈

    // 优先使用 lambda_handles 列表
    if (ast->lambda_handles && ast->lambda_handles->length > 0) {
        for (size_t i = 0; i < ast->lambda_handles->length; i++) {
            am_value_t h = am_list_get(ast->alloc, ast->lambda_handles, i);
            if (!am_value_is_handle(h)) continue;

            size_t depth = analyze_lambda_body(ast, am_value_to_handle(h));
            if (depth == SIZE_MAX) return SIZE_MAX;
        ...
        if (top_lambda == AM_HANDLE_NULL) return SIZE_MAX;

        size_t depth = analyze_lambda_body(ast, top_lambda);
        if (depth == SIZE_MAX) return SIZE_MAX;
        if (depth > max_depth) max_depth = depth;
    }
